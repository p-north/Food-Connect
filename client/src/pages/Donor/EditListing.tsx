...
const EditListing = () => {
  ...
  // Load the specific listing from localStorage on component mount
  useEffect(() => {
    const savedListings = localStorage.getItem('foodConnectListings');
    if (savedListings) {
      const parsedListings = JSON.parse(savedListings);
      const foundListing = parsedListings.find(l => l.id === parseInt(id));
      
      if (foundListing) {
        setListing(foundListing);
        setPreviewImage(foundListing.image);
      } else {
  ...
  const handleSubmit = (e) => {
    ...
    // Find and replace the current listing
      ...
      <div className="max-w-3xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        <div className="bg-white rounded-lg shadow-lg overflow-hidden">
          <div className="bg-green-600 px-6 py-4">
            ...
            <p className="text-green-100 mt-1">Update details of your food listing</p>
          </div>
