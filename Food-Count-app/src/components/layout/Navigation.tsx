  ...
  return (
    <nav className="bg-white shadow-sm">
      <div className="container mx-auto px-4 py-3 flex justify-between items-center">
        <Link to="/" className="flex items-center hover:scale-110 transition duration-300 ease-in-out">
        ...
        {isLoggedIn ? (
          ...
          <div className="flex items-center gap-4">
            <Link 
              to="/login" 
              className={`!text-black hover:text-gray-900 font-semibold ${location.pathname === '/login' ? 'text-black' : 'text-gray-800 hover:scale-110 transition duration-300 ease-in-out'}`}
            >
              Sign In
            </Link>
            <Link 
              to="/signup" 
              className={`hover:scale-110 transition duration-300 ease-in-out bg-green-500 !text-white px-4 py-2 rounded-md hover:bg-green-600 ${location.pathname === '/signup' ? 'bg-green-600' : ''}`}
            >
              Register
            </Link>
          </div>
